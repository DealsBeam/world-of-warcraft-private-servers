<!-- PROJECT OVERRIDES: wowprivateservers (WoW private-server tracker, Eleventy static) — pasted per taste docs (project style guide dominates on conflict). Do not remove on `skills update`; re-apply from git if overwritten. Decided Sep 2026, preserve-mode redesign shipped. -->
<!-- OVERRIDE 1 — identity (exempt from generic bans): dark-fantasy tracker for WoW players. Gold accent locked (`--accent`/`--gold`). Type stack Cinzel (display) + Crimson Pro (body) is brand-owned, not a multi-serif tell; Cinzel Decorative is wordmark-only. Era/metadata colors (vanilla/wotlk/tbc/etc.) are a semantic legend: the documented exception to the one-accent lock. -->
<!-- OVERRIDE 2 — documented near-black: `--bg #0c0b09` is warm charcoal chosen for gold legibility and torchlit identity, not a default reach. Radius scale is documented mixed (ctrl 4px / surface 10px / pill for the theme toggle only), which §4.4 permits. -->
<!-- OVERRIDE 3 — scope: redesigns here default to Preserve (levers 1-4). Content, URLs, nav labels, anchors, copy voice never change without explicit user approval. Em-dash ban applies to redesign-produced copy, NOT the 100+ hand-authored article bodies (archive). -->
<!-- OVERRIDE 4 — precedence: taste-skill v2 is the design authority; project AGENTS.md identity rules above win on conflict; frontend-design/impeccable/mobile-native/review-animations are dormant specialists (mobile checklist, animation gate), never design directors. -->
