...
- Supplied-source ingest: official Legacy, Skyborne, character creation, naming, ruleset, and hotfix pages were checked. Existing coverage already contains those claims; Sep 24 hotfix page has no Forever section. Blizzard's podcast article and official 62-minute video yielded one new discussion item, published as `news/forever-podcast-episode-1-sep-25.md`. Duskforge is Diablo 4-only. Icy Veins homepage and feeds returned Cloudflare 403.
- Follow-up gates: `npm test` passed with 147 news; `npm run build` passed with 421 files. CDN watcher read live 1.60.1.70009 with 6 bumps. Local 390px podcast-page QA passed with no overflow; local Vercel Insights 404 remains expected.
- Push was rebased over automated CDN commit `c70fc77`; content commit is now `9a11372` and pushed to `origin/main`. IndexNow accepted the podcast URL and feed with HTTP 200.
- Live podcast page initially showed a transient 404 during Vercel deployment, then returned HTTP 200. Live API count is 147 news; feed, API, sitemap, and llms include the podcast URL. Production desktop and 390px checks show no overflow and no current console errors.
- Reddit discovery: `Sharp_Classroom9686` posted a personal Classic Forever 1.60.1 project, level 20, beta-matched zones, no donations/shop, DM access. Public GitHub repo `defexnicolas/wow-classic-launcher` and status feed identify the same realm, report login/world online, and expose source plus SHA-256 release metadata. Direct login/world port probes accepted connections.
- Security hold: launcher source changes beta WTF config and patches client process memory; releases are unsigned, and comments mention an unverified earlier Google Drive batch. Added Classic Forever as `dev` with first-party status URL, not the private host. Published a security-aware news post with public GitHub source link; no executable or direct server domain linked.
- First build correctly rejected GitHub as a server URL because the domain gate treats every server host as private and existing GitHub links matched. Switched hidden tracker URL to the first-party raw status feed; rerun passed. Local server/news QA at 1280px and 390px showed no overflow, and built HTML contains no private host.
- Live deployment verified after push: Classic Forever server page and security news page HTTP 200, `/servers/` and API updated to 144 servers, news count 148, feed and sitemap include the new item, llms HTTP 200. Production 1280px and 390px QA shows no overflow; built/live HTML contains no private host. IndexNow accepted five affected URLs.
